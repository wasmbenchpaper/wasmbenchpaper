...
class Vertex:
	...
	def induce(self: "Vertex") -> "Graph":
		...
		#Graph.explore_iterative(self, induced, m, self.neighbours, time.time())
		Graph.explore_iterative(self, induced, m, self.neighbours, None)
		return induced
...
class Graph:
	...
	def duplicate(self: "Graph") -> "Graph":
		...
		for v in self.vertices():
			#Graph.explore_iterative(v, dup, m, None, time.time())
			Graph.explore_iterative(v, dup, m, None, None)
		"""
	...
	def colour_2(self: "Graph", i: int, j: int) -> bool:
		...
		for v in self.vertices():
			if v.colour.value != 0:
				continue
			if not Graph.colour_2_helper_iterative(v, i, j):
				return False
		return True
